...
#### 🔴 高優先級 (High Priority)
- [ ] **優化核心演算法以跨越鞍點**：修正 local min/max 自動平衡的最佳化邏輯，解決梯度下降會卡在「鞍點 (Saddle Point)」導致無效化的現象。
- [ ] **防止持續平衡的畫面震盪**：設計臨界點保護，避免色調在特定灰階分佈時，參數激烈跳動造成畫面閃爍。
- [ ] **UI 算圖效能與大圖卡頓排查**：評估 CPU 負責的 `QPixmap.fromImage` 負擔，在 4K 環境下考慮引入硬體加速或更極致的快取機制。

#### 🟡 中優先級 (Medium Priority)
- [ ] **修正背景線程在退出時的殘留**：排查 QThread 銷毀邏輯，確保 `OverlayWindow.closeEvent()` 徹底等待 `calc_worker` 釋放記憶體。
